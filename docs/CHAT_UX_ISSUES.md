...
Fix (`71ee31b`): `whenEmbeddingStatusKnown()` resolves the first non-unknown
...
changed in `88c2158` — but the UI-thread runtime still holds handlers
...
Update 2026-09-04 (`c099013`): seen again during a scripted device round
...
Fix (`1be825a`): once a reveal has run its fade, the container switches to
...
thing. The composer's send-echo guard (`42f374f`, this branch only — main
...
Confirmed on the Pixel 10 (`37e7d89`): send `test echo`, copy the user
...
deliberate — leaving is treated like Stop. Since `3c8a6c5`, Stop before the
