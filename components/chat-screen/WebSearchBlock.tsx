...
const WebSearchBlock = memo(({ isSearching, trace, results }: Props) => {
  ...
  const listRows = isSearching ? rows.slice(0, -1) : rows;
  const listVisible = listMounted && listRows.length > 0;
    ...
    >
      ...
      {currentRow ? (
        <Animated.View
          style={[
            styles.currentSlot,
            listVisible && styles.currentSlotAttached,
          ]}
