...
describe('search', () => {
  it('returns all models when search is empty', () => {
    const models = [
      makeModel({ modelName: 'Llama-3B' }),
      makeModel({ id: 2, modelName: 'Qwen-1B' }),
    ];
  ...
  it('applies all active filters together (AND logic)', () => {
    ...
    const { result } = renderHook(() =>
      useModelHubData({
        ...
        search: '',
        activeFilters: new Set([
          ModelHubFilter.Featured,
          ModelHubFilter.Downloaded,
        ]),
        groupByModel: false,
      })
    );
...
describe('groupByModel=false (default sections)', () => {
  it('splits into "Ready to Use" and "Available to Download"', () => {
    ...
    const ready = result.current.groupedModels.find(
      (g) => g.label === 'Ready to Use'
    )!;
    const available = result.current.groupedModels.find(
      (g) => g.label === 'Available to Download'
    )!;
...
describe('groupByModel=true (prefix grouping)', () => {
  it('groups models by name prefix', () => {
    ...
    const llamaGroup = result.current.groupedModels.find(
      (g) => g.label === 'llama'
    )!;
